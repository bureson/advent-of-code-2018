...
> Summary only - the original Part Two text is shown on adventofcode.com after Part One is solved.

You need to reach the target as fast as possible. You have a **torch** and a **climbing gear** and can use one of them or **neither**:
...
You start at `0,0` with the torch equipped. Moving to an adjacent region (up, down, left or right, never to negative `X` or `Y`) takes **one minute**, if the equipped tool is allowed in the new region. Switching the tool takes **seven minutes** and the new tool has to be allowed in the current region. The target has to be reached with the **torch** equipped. Regions beyond the target can be used.

In the example above, the fastest way takes `45` minutes.
